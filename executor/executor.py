...
Flow por sinal:
  ...
  3. Aplica lógica de sincronização (SyncLogic)
  4. Regra de borda (SyncState)
...
class Executor:
    ...
    async def process_signal(self, signal: Signal) -> ACK:
        ...
        # 5. Atualiza estado de sync (regra de borda)
        ...
        if decision == Decision.CLOSE:
            ...
            # Após fechar, verifica se deve reabrir em direção oposta
            if should_open and signal.direction != 0:
                open_ack = await self._open_position(signal, config)
                return open_ack
            return ack
    ...
    async def _open_position(self, signal: Signal, config: SymbolConfig) -> ACK:
        ...
        # Executa ordem
        # ⚠️ CONVERSÃO CRÍTICA: SL/TP financeiro (USD) → preço absoluto
        # A cTrader API (e maioria dos brokers) espera stopLoss/takeProfit como
        # preço absoluto, NÃO como valor em USD. Sem esta conversão, um SL de
        # $10.0 seria interpretado como stop no nível de preço 10.0, o que é
        # completamente errado para qualquer par forex.
        # Ver: executor/price_converter.py, SPEC_CONNECTOR.md §4.2
    ...
    async def _get_current_price(self, symbol: str) -> float:
        ...
        Prioridade:
          1. Posição aberta (current_price mais recente)
          2. Último preço conhecido pelo Connector
          3. 0.0 (fallback — PriceConverter usará estimativas)
        """
        ...
        # Tenta todas posições (para mock/tests)
        try:
